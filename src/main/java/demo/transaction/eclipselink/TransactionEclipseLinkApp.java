package demo.transaction.eclipselink;
 ...
 */
public class TransactionEclipseLinkApp {
    ...
    private void loadTestingData() {
        Account a = dao.findById(TransactionEclipseLinkApp.ACC_A);
        if (null == a) {
            a = new Account(TransactionEclipseLinkApp.ACC_A);
            a.setAmount($.random(IntRange.of(100, 2000)));
            dao.save(a);
        }
        Account b = dao.findById(TransactionEclipseLinkApp.ACC_B);
        if (null == b) {
            b = new Account(TransactionEclipseLinkApp.ACC_B);
